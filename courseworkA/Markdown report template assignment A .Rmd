...
The conceptual model related to the research question can be viewed in \autoref{fig:concept_mocel}. The main question is about the effect of mutual visibility on joint laughter. The mediating variables are the duration and the frequency of the laughter, since that is how we will have to express joint laughter in order to arrive at a conclusion. The moderating variable is familiarity, which we can define as the level of friendliness or intimacy between people. This can be caused when people know each other (which we aim to avoid), but also when people find similarities in their interests and behaviours. It is even possible that people of the same gender will feel more familiar with each other. 
...
## Measures
The data that has been collected includes audio and video of participants. The first step in data analysis involves annotating the signals. This will be done with a program in which we can manually select timesteps in which the participant is laughing, containing a duration of that laughter. In the end we will have annotations for every person that contains the total amount of laughter (frequency), the amount of laughter with their webcam of, the amount of laughter with their webcam on, and the duration of every laughter episode.
