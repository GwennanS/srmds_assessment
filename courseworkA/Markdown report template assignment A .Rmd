...
For model1, the model with session as a factor, we take as prior a normal distribution of N(125,30). This comes from the mean of the score, 125, and a bit more than the standard deviation, which is around 27. Our sigma is set at a uniform distribution of U(0.001,30).

$$ score \sim Norm(\mu, \sigma)$$ <!-- [Likelihood]-->

$$ \mu = a + b_{session} $$ <!--[linear model]-->

$$ a_{clinic} \sim Norm(0,a_{\sigma})$$ <!--[adaptive prior]-->

$$ a_{\sigma} \sim Caucy(0,10)$$<!--[hyper prior]-->

$$ a \sim Norm(125,25)$$ <!--[fixed prior]-->

$$ \sigma = Caucy(0, 90)$$ <!--[fixed prior]-->
...
m0 <- map2stan(
  alist(
    ...
    # fixed prior
    a_Subject[Subject] ~ dnorm(125,25), 
    sigma ~dcauchy(0,90)
...
m1 <- map2stan(
  alist(
    ...
    # adaptive prior
    a_Subject[Subject] ~ dnorm(125,sigma_subject),
...
From the previous comparison we could see that model2 is the best fit model. We will further examine this model with 95% credible intervals of the parameters of this model. We take a look at the estimated values of the parameters of this model

```{r}
pander(precis(m2, depth=2, prob=.95))
```
